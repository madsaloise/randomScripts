...
function WST_GetSwingTimers()
    return addon_data.player.main_swing_timer, addon_data.player.off_swing_timer
end

function WST_GetSlamDelay()
    return character_warrior_settings and character_warrior_settings.slam_delay or 0.1
end

-- Returns the current main-hand weapon speed in seconds (haste-adjusted).
function WST_GetMainWeaponSpeed()
    return addon_data.player.main_weapon_speed or 2
end
...
function addon_data.player.UpdateVisualsOnUpdate()
    ...
    if settings.enabled and (
        addon_data.player.has_twohand and settings.enable_twohanding or
        addon_data.player.has_offhand and settings.enable_dualwielding or
        not (addon_data.player.has_twohand or addon_data.player.has_offhand) and settings.enable_onehanding
    ) then
...
function addon_data.player.OneHandingCheckBoxOnClick(self)
    character_player_settings.enable_onehanding = self:GetChecked()
    addon_data.player.UpdateVisualsOnSettingsChange()
end

function addon_data.player.DualWieldingCheckBoxOnClick(self)
    character_player_settings.enable_dualwielding = self:GetChecked()
    addon_data.player.UpdateVisualsOnSettingsChange()
end

function addon_data.player.TwoHandingCheckBoxOnClick(self)
    character_player_settings.enable_twohanding = self:GetChecked()
    addon_data.player.UpdateVisualsOnSettingsChange()
end
